let addcl (a1, a2) = match (a1, a2) with
                    (CLOSURE(t1, Const aa1), CLOSURE(t2, Const aa2)) ->
                        CLOSURE([], Const(aa1+aa2))
;;

let subcl (a1, a2) = match (a1, a2) with
                    (CLOSURE(t1, Const aa1), CLOSURE(t2, Const aa2)) ->
                        CLOSURE([], Const(aa1-aa2))
;;

let divcl (a1, a2) = match (a1, a2) with
                    (CLOSURE(t1, Const aa1), CLOSURE(t2, Const aa2)) ->
                        CLOSURE([], Const(aa1/aa2))
;;

let mulcl (a1, a2) = match (a1, a2) with
                    (CLOSURE(t1, Const aa1), CLOSURE(t2, Const aa2)) ->
                        CLOSURE([], Const(aa1*aa2))
;;

let modcl (a1, a2) = match (a1, a2) with
                    (CLOSURE(t1, Const aa1), CLOSURE(t2, Const aa2)) ->
                        CLOSURE([], Const(aa1 mod aa2))
;;

let abscl a1 = match a1 with
                CLOSURE(t1, Const aa1) -> if aa1<0 then
                                            CLOSURE([], Const (-aa1))
                                          else
                                            CLOSURE([], Const aa1)
;;

let notcl a1 = match a1 with
                CLOSURE(t1, True) -> CLOSURE([], False)
            |   CLOSURE(t1, False) -> CLOSURE([], True)
;;

let andcl (a1, a2) = match (a1, a2) with
                        (CLOSURE(t1, True), CLOSURE(t2, True)) ->
                            CLOSURE([], True)
                    |   _ -> CLOSURE([], False)
;;

let orrcl (a1, a2) = match (a1, a2) with
                    (CLOSURE(t1, False), CLOSURE(t2, False)) ->
                        CLOSURE([], False)
                |   _ -> CLOSURE([], True)
;;

let implycl (a1, a2) = match (a1, a2) with
                        (CLOSURE(t1, True), CLOSURE(t2, False)) ->
                            CLOSURE([], False)
                    |   _ -> CLOSURE([], True)
;;

let greatcl (a1, a2) = match (a1, a2) with
                        (CLOSURE(t1, Const aa1), CLOSURE(t2, Const aa2)) ->
                            CLOSURE([], if (aa1>aa2) then True else False)
;;

let equalcl (a1, a2) = match (a1, a2) with
                        (CLOSURE(t1, Const aa1), CLOSURE(t2, Const aa2)) ->
                            CLOSURE([], if (aa1=aa2) then True else False)
;;

let lessscl (a1, a2) = match (a1, a2) with
                        (CLOSURE(t1, Const aa1), CLOSURE(t2, Const aa2)) ->
                            CLOSURE([], if (aa1<aa2) then True else False)
;;

let grequcl (a1, a2) = match (a1, a2) with
                        (CLOSURE(t1, Const aa1), CLOSURE(t2, Const aa2)) ->
                            CLOSURE([], if (aa1>=aa2) then True else False)
;;

let leequcl (a1, a2) = match (a1, a2) with
                        (CLOSURE(t1, Const aa1), CLOSURE(t2, Const aa2)) ->
                            CLOSURE([], if (aa1<=aa2) then True else False)
;;

let rec expocl (a1, a2) = match (a1, a2) with
                            (CLOSURE(t1, Const aa1), CLOSURE(t2, Const aa2)) -> CLOSURE([], Const (poww aa1 aa2))
;;

let rec krivine c s = match (c, s) with
                        (CLOSURE(gamma, True), s) -> CLOSURE(gamma, True)
                    |   (CLOSURE(gamma, False), s) -> CLOSURE(gamma, False)
                    |   (CLOSURE(gamma, Const(n)), s) -> CLOSURE(gamma, Const(n))
                    |   (CLOSURE(gamma, Abs(n)), s) -> CLOSURE(gamma, Abs(n))
                    |   (CLOSURE(gamma, Not(n)), s) -> CLOSURE(gamma, Not(n))
                    |   (CLOSURE(gamma, Projj(i, List(ex))), s) -> krivine (CLOSURE(gamma, ith(i, ex))) []
...
let rec map_kr l env = match l with
                        [] -> []
                    |   x::y -> (krivine (CLOSURE(env, x)) [])::(map_kr y env)
;;

let rec map_cl l env = match l with
                        [] -> []
                    |   x::y -> CLOSURE(env, x)::(map_cl y env)
;;

let rec give_me_answers cl = match cl with
                            CLOSURE(_, True) -> Bool(true)
                        |   CLOSURE(_, False) -> Bool(false)
                        |   CLOSURE(_, Const(n)) -> Int(n)
                        |   CLOSURE(e, List(l)) -> Listans (map give_me_answers (map_cl l e))
;;
