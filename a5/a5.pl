% ht stands for hastype: (G, E, T);
% true if Type(E)=T.
% :%s/ht/hastype/g
% te stands for typeElaborate: (G, D, G');
% true if G' is (D augmented G)
% :%s/te/typeElaborate/g
% dv stands for definedVariables: (D, L);
% true if L is list of dv(D).
% :%s/dv/definedVariables/g

% lookup: (L, F, S); true if (F, S) in L.
% Should have just used member -_-
% fixed ^
% ith: (I, L, Ei); true if L[I]=Ei. For projection
% member: (X, L); true if X is in L.
% intersection: (L1, L2);
% true if L1, L2 don't have any common elements.
...
% derived: T1->T2(arrowT), T1*T2...*Tn(cartesianT)
...
ht(G, anD(E1, E2), boolT):-
    ht(G, E1, boolT),
    ht(G, E2, boolT).
ht(G, orr(E1, E2), boolT):-
    ht(G, E1, boolT),
    ht(G, E2, boolT).
ht(G, imply(E1, E2), boolT):-
    ht(G, E1, boolT),
    ht(G, E2, boolT).
...
% abstraction,
ht(G, lambda(v(X), E), arrowT(T1,T2)):-
    ht([p(v(X), T1)|G], E, T2).

% function application
ht(G, apply(E1, E2), T2):-
    ht(G, E2, T1),
    ht(G, E1, arrowT(T1, T2)).
...
dv(seq(D1, D2), L):-
    dv(D1, L1), dv(D2, L2), append(L1, L2, L).
dv(pll(D1, D2), L):-
    dv(D1, L1), dv(D2, L2), intersection(L1, L2),
    append(L1, L2, L).
